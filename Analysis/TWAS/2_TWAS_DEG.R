...
outdir <- "./2_TWAS_DEG.outs/Correct_results/"
if (!file.exists(outdir)) dir.create(outdir, showWarnings=F)


###
### Twas Traits
### Last modified Nov-7-2022, by Julong wei
...
x2 <- openxlsx::read.xlsx(fn)

##
gene_comb2 <- rbind(gene_comb2, x2)
...
category <- sort(unique(gene_comb2$category))
DF <- NULL
...
###
plotDF2 <- plotDF%>%filter(!category%in%c("Union_twas", "addiction"))
...
### 
figfn <- paste(outdir, "Figure1.1_bubble.png", sep="")
...
traits <- c("addiction", "ADHD", "alcohol", "BMI", "CAD", "caffeine", "height",  "marijuana", "parkinson", "smoke")
...
for (oneMCl in celltypes){
    ##
    BG <- resDiff%>%drop_na(p.value)%>%filter(celltype==oneMCl)%>%pull(gene)%>%unique()
    BG <- intersect(BG, allgene)
    DEG <- resDiff%>%drop_na(p.value)%>%
        filter(celltype==oneMCl, p.adjusted<0.1, abs(estimate)>0.25)%>%pull(gene)%>%unique()
    DEG <- intersect(DEG, allgene)
    
    if ( length(DEG)==0) next
    
    ###
    for ( ii in category){
        ##
        twas_gene <- gene_comb2%>%filter(category==ii)%>%pull(SYMBOL)%>%unique()
        ##
        DEG_1 <- intersect(DEG, twas_gene)
        DEG_2 <- intersect(DEG, setdiff(allgene, twas_gene))
        ##
        notDEG_1 <- intersect(setdiff(BG, DEG), twas_gene)
        notDEG_2 <- intersect(setdiff(BG, DEG), setdiff(allgene, twas_gene))       
        ###                   
        df2 <- data.frame(MCls=oneMCl, category=ii,
                          n_DEG_1=length(DEG_1), n_notDEG_1=length(notDEG_1),
                          n_DEG_2=length(DEG_2), n_notDEG_2=length(notDEG_2))
       DF <- rbind(DF, df2)
   }
}        
...
DF <- DF%>%mutate(comb=paste(category, MCls, sep="_"))
comb2 <- sort(unique(DF$comb))

plotDF <- NULL
for ( ii in comb2){
   ## 
   DF2 <- DF%>%filter(comb==ii)
   dat <- matrix(as.numeric(DF2[,3:6]), 2, 2)
   pval <- as.numeric(fisher.test(dat, alternative="greater")$p.value)
   fisher <- fisher.test(dat)
   res2 <- data.frame(odds=as.numeric(fisher$estimate),
       CI_upper=as.numeric(fisher$conf.int[2]), CI_lower=as.numeric(fisher$conf.int[1]),
       pval=pval)
   ##
   plotDF <- rbind(plotDF, cbind(DF2, res2))
}    
    
####
####
plotDF2 <- plotDF%>%mutate(MCl_value=MCls_val[as.character(MCls)],
                          MCls=fct_reorder(MCls, MCl_value))
plotDF2 <- plotDF2%>%filter(odds>0)

p <- ggplot(plotDF2, aes(x=odds, y=comb, color=factor(MCls)))+
   geom_errorbarh(aes(xmax=CI_upper, xmin=CI_lower), size=0.5, height=0.2)+
   geom_point(shape=19, size=1.5)+
   scale_colour_manual(values=col_MCls,
      breaks=c("ODC", "Astrocyte", "Microglia", "OPC", "Non_DA", "Pericyte"))+
   xlab("Odds ratio")+
   ##scale_y_discrete()+ 
...
figfn <- paste(outdir, "Figure1.2_odds_forestplot.png", sep="")
png(filename=figfn, width=650, height=580, res=120)  
print(p)
dev.off()
